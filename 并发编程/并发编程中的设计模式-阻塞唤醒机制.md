...
```java
package com.jyc.demo.concurrent.park;
...
\_mutex[1]和\_cond[2]都是为了配合完成c++层面线程的阻塞与互斥等操作。
...
## ObjectMonitor中wait/notify方法的实现

```cpp
void ObjectMonitor::wait(JavaThread* current, TRAPS) {
    assert(current == JavaThread::current(), "must be");
    assert(current->is_Java_thread(), "must be");
    assert(current->is_parked(), "must be parked");

    // 释放锁并将线程添加到等待队列
    release(current);
    add_to_wait_set(current);
    current->park();
}
```
...
void JavaThread::park() {
    assert(is_Java_thread(), "must be Java thread");
...
objectMonitor :https://hg.openjdk.org/jdk8/jdk8/hotspot/file/87ee5ee27509/src/share/vm/runtime/objectMonitor.hpp
